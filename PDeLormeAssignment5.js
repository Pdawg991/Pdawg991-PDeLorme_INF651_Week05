...
function getPizzaType(pizzaArr){
return pizzaArr[0];
}
...
function addTax(pizzaArr){
    pizzaArr.splice(2, 0, Math.round(pizzaArr[1]*.0925*100)/100);
    return pizzaArr;
    }
...
function isPalindrome (string){
    stringInput = string.toLowerCase().replaceAll(" ", "");
    stringCopy = string.toLowerCase().replaceAll(" ", "");
    let x = 0;
    for (i = stringInput.length-1; i > 0; i--){
       
       if(stringInput[i] != stringCopy[x]){
